...
public class FaceDetectorRetina extends FaceDetector {
    ...
    public FaceDetectorRetina(Activity activity, int h, int w, int c, float s, String model_name) throws IOException {
        super(activity, h, w, c, model_name); // 부모 호출
    ...
    public Vector<boundingBox> detect(byte[] img_in, long[] times){
        // 모델에 맞는 input이 들어올 수 있도록 데이터에 전처리를 가하는 단계입니다.
        // 이미지 리사이징 작업
        imgData.rewind(); // 시작

        startTime = SystemClock.uptimeMillis(); // 코드의 속도체크 시작
        // img_in은 input data h,w,c
        for(int i = 0; i < h; i++){
            for(int j = 0; j < w; j++){
                for(int k = 0; k < c; k++){
        ...
        endTime = SystemClock.uptimeMillis(); // 코드의 속도 체크 종료
        times[0] = endTime-startTime;  // 코드의 진행시간을 확인하기위한 변수
        // 이미지 리사이징 종료
        Object[] inputArray = {imgData};  // 위의 과정을 거치고 리사이징된 input데이터를 Object[]로 만듬

        Map<Integer, Object> outputMap = new HashMap<>(); // Integer와 Object를 담을수있는 Map객체 생성
