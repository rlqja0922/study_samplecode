...
public class CameraNcnnFragment extends Fragment
    ...
    // handler对象，用来接收消息     //핸들러 대상, 메시지 수신
    private Handler imgupdatehandle = new Handler() {
        @Override
        public void handleMessage(android.os.Message msg) {
            // 处理从子线程发送过来的消息    //서브라인에서 전송된 메시지 처리하기
    ...
    private final ImageReader.OnImageAvailableListener mOnImageAvailableListener
            = new ImageReader.OnImageAvailableListener() {      // 실시간으로 이미지 가져오기.
        ...
        public void onImageAvailable(ImageReader reader) {
            ...
            //split process function to prevent img reflash stop    //프로세스 기능을 분할하여 이미지 리플레쉬가 중지되지 않도록 합니다.
            if(!mDetect_isbusy)
            {
                mDetect_isbusy = true;
                new Thread(() -> {
                    float[] result = ncnnprocess_imgyuv(yuv, width, height);    // 얼굴 값.
                    if(result != null) {
                        mDetect_result = Arrays.copyOfRange(result, 128, result.length);  // copyOfRange( 원본 배열, 시작 인덱스, 마지막 인덱스): 배열 복사
    ...
    public void onViewCreated(final View view, Bundle savedInstanceState) {
        ...
        captureface = (ImageView)view.findViewById(R.id.captureface);   // CaptureFace 보여주는 것
        targetface = (ImageView)view.findViewById(R.id.targetface); // 보여주는 것
        mSurfaceView = (AutoFitSurfaceView) view.findViewById(R.id.texture);
        resulttextview = (TextView)view.findViewById(R.id.resulttext);  // 인식률.
    }
    ...
    private void setUpCameraOutputs(int width, int height) {
        Activity activity = getActivity();
        CameraManager manager = (CameraManager) activity.getSystemService(Context.CAMERA_SERVICE);  // 카메라 열기.
        try {
    ...
    private void openCamera(int width, int height) {
        if (ContextCompat.checkSelfPermission(getActivity(), Manifest.permission.CAMERA)    // 카메라 권한.
    ...
    @Override
    public void onClick(View view) {    // 누르면 화면의 CaptrueFace 이미지를 TagetFace로 이동
        switch (view.getId()) {
            //
            case R.id.capture: {
