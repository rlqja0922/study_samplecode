...
public class CameraActivity extends AppCompatActivity implements CameraBridgeViewBase.CvCameraViewListener2 {
    ...
    public Mat onCameraFrame(CameraBridgeViewBase.CvCameraViewFrame inputFrame) {
        mRGBA = inputFrame.rgba(); // 이미지 mat객체를 반환합니다.

        MatToBitmap(mRGBA); //이미지 변환, 이미지 회전
        recognize_face();
...
//        box = b.get(0);
    ...
    private void MatToBitmap(Mat mat) {
        try {
            camera_frame_bitmap = Bitmap.createBitmap(mat.cols(), mat.rows(), Bitmap.Config.ARGB_8888); //비트맵 생성
            Utils.matToBitmap(mat, camera_frame_bitmap); // Mat을 비트맵으로 변환

            matrix = new Matrix();
            if (JavaCameraView.isFrontFacing)
                matrix.postRotate(270); //이미지 회전
            else
