...
//        String storagePath = module.equals("register") ? Environment.DIRECTORY_PICTURES : Environment.DIRECTORY_PODCASTS;
        ...
        if (takePictureIntent.resolveActivity(getPackageManager()) != null) {
            try {
                ...
                File storageDir = getExternalFilesDir(storagePath); //외부저장소
                File image = File.createTempFile("JPEG_" + timeStamp + "_", ".jpg", storageDir); //캐시영역에 파일저장
                currentphotoUri = FileProvider.getUriForFile(this, FILE_PROVIDER_PATH, image);
                currentphotoFile = image;

                // Continue only if the File was successfully created
                // 안드로이드 7.0 이상부터는 보안상 이슈로 FileProvider.getUriForFile 이용하여 URI 정보를 획득
                if (image != null) {
    ...
    private void register_face(Uri uri, String module, String person_name) {
        try {
            ...
            imageView.setImageBitmap(bitmap); // 이미지뷰에 bitmap파일을 출력
            // 얼굴 검출메소드 MTCNN 사용
...
//                File cropped = File.createTempFile(person_name, ".jpg", storageDir);
                ...
                bitmap.compress(Bitmap.CompressFormat.JPEG, 100, out); //해당 형식으로 압축
                out.flush();
                out.close(); // 스트림 종료
...
//    이름을 등록하는 메서드
    ...
    private void recognize_face(Uri uri, String module) {
        try {
            ...
            imageView.setImageBitmap(new_image); // 이미지뷰에 bitmap파일을 출력
            // 얼굴 검출메소드 MTCNN 사용 비교 인식을위한 new_image
