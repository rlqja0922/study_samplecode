...
class NFCViewController: UIViewController {
    ...
    //샘플 앱은 사용자가 스캔 버튼을 탭하면 리더 세션을 시작합니다. 앱은 첫 번째 태그를 읽은 후 세션을 무효화하도록 리더 세션을 구성합니다. 추가 태그를 읽으려면 사용자가 스캔 버튼을 다시 탭합니다.
  ...
  //  NDEF 메시지를 검색할 때마다 세션은 메서드 를 호출하여 대리자에게 메시지를 보냅니다 . 이것은 데이터로 유용한 일을 할 수 있는 앱의 기회입니다. 예를 들어 샘플 앱은 사용자가 나중에 볼 수 있도록 메시지를 저장합니다.
...
extension NFCViewController: NFCNDEFReaderSessionDelegate {
    ...
    //리더 세션이 종료되면 대리자 메서드를 호출하고 세션 종료 이유를 제공하는 오류 개체를 전달합니다.
